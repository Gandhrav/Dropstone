...
```text
$ python capture.py "bought Atomic Habits for 15 bucks, excited to read it"

raw:    bought Atomic Habits for 15 bucks, excited to read it
note_id: 14
routed:
  - expense (confidence=0.85)
  - book (confidence=0.90)
stored:
  - expense row #7: {"amount": 15.0, "currency": "USD", "category": "books", ...}
...
flowchart LR
    ...
    S -->|"idea · research · book"| KN["knowledge"]
    S -.->|"no match"| IB["📮 inbox<br/>manual triage"]
    TR --> DB[("SQLite<br/>poc.db")]
    TA --> DB
    KN --> DB
...
|---|---|---|
| `ANTHROPIC_API_KEY` | Router + digest LLM calls | *(required)* |
